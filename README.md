...
<p align="center">
  ...
  &nbsp;·&nbsp;
  <b><a href="docs/blog/tool-calling.md">→ 博客：多步工具调用 + 可视化</a></b>
  &nbsp;·&nbsp;
  <b><a href="docs/blog/rag-frontend.md">→ 博客：纯前端 RAG</a></b>
</p>
...
- **纯前端 RAG** — 本地 embedding（Transformers.js + all-MiniLM-L6-v2）+ IndexedDB 向量库 + cosine 检索，零 API 成本，数据不出本机
- **虚拟滚动** — react-virtuoso，万条消息对话保持流畅
- **按需加载** — markdown 渲染器、Transformers.js 都首屏不加载
- **完整工程链路** — Vitest 单测（37 个）+ Playwright E2E（3 个）+ GitHub Actions CI
...
- `lib/rag/chunk.ts` — 中英文句末切分、overlap、极长串硬切
- `lib/rag/retrieve.ts` — cosine 数学性质、top-k 排序、边界情况
...
点顶部「📎 知识库」打开侧面板，上传 `.md` / `.txt`，开启"本次对话使用检索增强"开关，提问时会自动检索最相关片段并注入 system prompt。
...
文件 (md/txt)
  → File.text()
  → chunkText()          lib/rag/chunk.ts
  → Transformers.js      lib/rag/embedder.ts   (浏览器里跑 all-MiniLM-L6-v2)
  → Float32Array
  → IndexedDB            lib/db.ts (ragChunks 表，embedding: ArrayBuffer)

提问
  → embed(query)
  → cosineSimilarity top-4   lib/rag/retrieve.ts
  → 拼到 system prompt
  → POST /api/chat
```
...
- **模型全局单例** — `getEmbedder()` 懒加载 + `Promise` 缓存，首次 ~25MB 下载，之后走 IndexedDB 缓存
- **向量存 ArrayBuffer** — 比 `number[]` 省 ~80% IndexedDB 空间
- **向量归一化** — `{ pooling: "mean", normalize: true }` 让余弦相似度等价于点积
- **`experimental_prepareRequestBody`** — 发送前 async 检索好，同步塞进 body
...
- [x] 纯前端 RAG（Transformers.js + IndexedDB 向量检索）
- [ ] 流式期间实时持久化（修断电丢回复）
- [ ] 多模态上传（图片 / PDF + Claude Vision）
- [ ] 纯前端 RAG PDF 支持（pdfjs-dist）
- [ ] 命令面板 `Cmd+K` + 键盘快捷键
